...
statistics" dans `CLAUDE.md`. Le canal de mutation SNI et la statistique
`AML` (à 3 populations, hors de portée de nos jeux de test à 2
populations) restent différés ; le rejeu `_from_values` et les 11
stats n'ont pas encore été comparés à un vrai reftable DIYABC microsat
(aucun disponible pour l'instant). Un test connu reste à corriger
(`tests/test_pipeline.py::test_compute_summary_statistics_microsat`,
attend encore l'ancien `NotImplementedError`).
