...
from bridge.ancestry_simulation import build_samples_argument, simulate_independent_loci, _draw_single_mutation_node
import random
...
def draw_fast(tree, ts, rng):
    ...
    if idx >= len(nodes): idx = len(nodes)-1
...
def draw_vec(ts, rng):
    ...
    target = rng.uniform(0, total)
    idx = np.searchsorted(np.cumsum(lengths), target)
    if idx >= len(edges.child): idx = len(edges.child)-1
